 ...
 *
 * Arguments:
 *      in - input data block encoded 0x00RRGGBB (8 pixel padded) - Mapped to
 *          one port BRAM
 *      out - output data block encoded 0x00RRGGBB (non 8 pixel padded) - Mapped
 *          to one port BRAM
 *      sw0 - mapped to external signal from the board (pin M20)
 *      sw1 - mapped to external signal from the board (pin M19)
 *
 * Returns:
 *      [THOUGH POINTER] - output data block
 * Side-Effects:
 *      [NONE]
 *
 * Description:
 *      Top function.
 *      Multiplexes the correct to be applied to in.
 ******************************************************************************/
 ...
 *
 * Description:
 *      Applies the checkered filter to in and stores the
 *      result in out.
 ******************************************************************************/
void checkered(unsigned in[BHEIGHT][BWIDTH], unsigned out[UHEIGHT][UWIDTH]){
	int i,j;
 ...
 *
 * Description:
 *      Applies the frame filter to in and stores the
 *      result in out.
 ******************************************************************************/
void frame(unsigned in[BHEIGHT][BWIDTH], unsigned out[UHEIGHT][UWIDTH]){
	int i,j;
 ...
 *
 * Description:
 *      Applies the Convulution radius 4 (Blur) filter to in and stores the
 *      result in out.
 ******************************************************************************/
void naive_avg_Conv4(unsigned in[BHEIGHT][BWIDTH], unsigned out[UHEIGHT][UWIDTH]){
    signed i, j;
 ...
 *
 * Description:
 *      Applies the Convulution radius 8 (Blur) filter to in and stores the
 *      result in out.
 ******************************************************************************/
void naive_avg_Conv8(unsigned in[BHEIGHT][BWIDTH], unsigned out[UHEIGHT][UWIDTH]){
    signed i, j;
 ...
 *
 * Description:
 *      Applies the Convulution radius 4 and 8 (Blur) filter to in and stores
 *      the result in out.
 *      Minimizes the read accesses, since it applies the pixel being read to
 *      all kernels which depend on it.
 * 
 * Observations:
 *      Abandonned since the accumulator array was being mapped to BRAM
 *      Loop limits need to be fixed to a constant, requirement from the tool
 *      being used, Vitis HLS
 ******************************************************************************/
// void avg_Conv( unsigned in[BHEIGHT][BWIDTH], unsigned out[UHEIGHT][UWIDTH], unsigned offset){
